// src/app/api/teacher/student-messages/[id]/route.ts
// POST: ردّ المدرّس على رسالة طالب، ويُشعَر الطالب. الملكية: مالك اختبار الجلسة فقط.
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const msg = await prisma.studentMessage.findUnique({
    where: { id: params.id },
    select: {
      id: true,
      studentId: true,
  ...
  try {
    await createNotification({
      ...
      type: "student_message_reply",
      message: `ردّ مدرّسك على رسالتك بخصوص «${msg.session.quiz.title}».`,
      linkUrl: `/student/quizzes/${msg.session.quizId}`,
    });
  } catch {
    // تجاهل.
  }
